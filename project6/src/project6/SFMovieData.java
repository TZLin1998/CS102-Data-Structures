			...
			while ((str = in.readLine()) != null) {
				...
				try {
					...
					if(sw.get(9) != null && sw.get(10) != null
							&& !sw.get(9).equals("") && !sw.get(10).equals("")) {
						...
						if(sw.get(9) != null && !sw.get(9).equals("")) {
							Actor a2 = new Actor(sw.get(9));
					...
					//if the current movie is in the list, add its location
					if(list.contains(current)) {
						while(listIterator.hasNext()) {
							Movie m = listIterator.next();
							if (m.getTitle().toLowerCase().equals(title.toLowerCase())) {
								//add new location to the movie
								m.addLocation(loc);
							}
						}
					}else { // if movie is not in the list, add it
